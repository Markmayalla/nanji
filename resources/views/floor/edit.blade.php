        ...
        <div class="animated fadeIn">
            <div class="card col-md-12">
                ...
                <div class="card-body">
                    ...
                    <form class="form" method="POST" action="{{ route('floor.update',['id' => $floor->id]) }}">
                        {{ csrf_field() }}
                        @method('PUT')
                                ...
                                @if(contain_id($field))
                                    <select name="{{replace_id($field)}}" id="{{replace_id($field)}}" class="form-control">
                                        ...
                                        @foreach($$variables as $d)
                                            <option {{$floor->$field == $d->id ? 'selected' : '' }} value="{{$d->id}}">{{$d->name}}</option>
                                        @endforeach
                                    </select>
                                    ...
                                    {!! "@endif" !!}
                                @else
                                    <input type="text" name="{{$field}}" value="{{$floor->$field}}" id="{{$field}}" class="form-control" placeholder="Enter {{ ucfirst(str_replace('_',' ',replace_id($field)))}}" />
                                @endif
                                {!! "@if ($errors->has('".$field."'))" !!}
